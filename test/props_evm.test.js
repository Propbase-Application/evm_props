  ...
  : describe("PROPS TESTS", function () {
      ...
      describe("Basic Functionality Test", function () {
        it("Initializes the PROPS Correctly.", async () => {
          ...
          const is_admin = await props.hasRole(ethers.keccak256(ethers.toUtf8Bytes("ADMIN_ROLE")), deployer.address);
          assert.equal(is_admin, true);

          const decimals = await props.decimals();
          assert(decimals, 8);

          const DEFAULT_ADMIN_ROLE = '0x' + '00'.repeat(32);
          const is_defualt_admin = await props.hasRole(DEFAULT_ADMIN_ROLE, deployer.address);
